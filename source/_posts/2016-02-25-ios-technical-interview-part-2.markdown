...
###7. 怎么用 copy 关键字？  
A:首先是对象所属的类遵守 NSCopying 协议，其次是我们想拥有赋值对象的副本，这时我们可以使用 copy 关键字。   
...
###37. 以`+ scheduledTimerWithTimeInterval...`的方式触发的timer，在滑动页面上的列表时，timer会暂停回调，为什么？如何解决？  
...
本质区别：
类方法是作用在类对象上，而实例方法作用在实例对象上。

联系：
实例对象是由作用在类对象上的类方法创建的，只有先调用类方法创建实例对象，才有机会调用实例对象的实例方法。

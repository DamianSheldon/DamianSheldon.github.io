...
在 Spring MVC 里，控制器（Controller） 就是用来接收浏览器请求、处理参数、调用业务逻辑，并返回响应结果的核心组件。通常是由 `@Controller` 注解的类。

## Q21. `@RequestMapping` 注解是如何工作的？
`@RequestMapping` 注解用于将 Web 请求映射到Spring 控制器方法。除了简单的用例之外，我们还可以用它来映射 HTTP 头，
用 `@PathVariable` 来绑定URI的部分内容，以及用 URI 参数和 `@RequestParam` 注解来工作。
...
译者点评：个人觉得这个题出得不怎么好，给的答案也有点答非所问。单纯说 `@RequestMapping` 注解是如何工作的？
那答案应该重点说`@RequestMapping` 注解会将映射请求所需的匹配信息保留到 Java 运行时，
出题者更多想考察的应该是 Spring MVC 是如何将请求映射到 `@RequestMapping` 注解的方法。

@RequestMapping 是 Spring MVC 中映射 HTTP 请求与控制器方法的核心注解，它的工作逻辑可以概括为 3 步：

* 注册映射关系：项目启动时，Spring 会扫描所有标注了 @RequestMapping（包括 @GetMapping/@PostMapping 等派生注解）的控制器类 / 方法，
把「请求路径 + 请求方法」和对应的方法绑定，存入一个请求映射注册表；
* 接收并匹配请求：当客户端发送 HTTP 请求时，DispatcherServlet（前端控制器）会提取请求的「URL 路径」和「请求方法（GET/POST）」，
去注册表中查找匹配的控制器方法；
* 分发执行：找到匹配的方法后，DispatcherServlet 会调用该方法，处理参数、执行业务逻辑，最后把结果返回给客户端。

另外，@RequestMapping 可加在类上（定义基础路径），也可加在方法上（定义具体路径），两者组合就是完整的请求地址。

# 4. Spring Data Access

## Q22. 什么是 Spring JdbcTemplate 类以及如何使用它？
Spring JdbcTemplate 是数据库操作主要的API，我们可以通过它访问我们感兴趣的数据：
...
AOP 的这四个核心概念，核心是为了实现「在指定位置执行指定增强逻辑」，用通俗的话讲：

* JoinPoint（连接点）：程序执行过程中可以被增强的所有位置，比如方法的调用、执行、异常抛出等（Spring AOP 中主要指方法的执行）；
* Pointcut（切入点）：实际要增强的 JoinPoint（筛选后的连接点），比如 “所有 Service 层的 save 方法” 就是一个切入点；
* Advice（通知 / 增强）：要在切入点执行的具体逻辑，比如日志记录、事务控制、参数校验这些功能；
* Aspect（切面）：Pointcut + Advice 的组合，是 AOP 的核心载体，比如 “在 Service 层所有 save 方法执行前记录日志”，这整个规则就是一个切面。

## Q27. 什么是编织？
根据[官方文档](https://docs.spring.io/spring/docs/current/spring-framework-reference/html/aop.html)，编织是一个将各个切面与其他应用程序类型或对象联系起来以创建一个增强对象的过程。这可以在编译时、加载时或运行时完成。
Spring AOP 和其他纯 Java AOP 框架一样，在运行时执行织入。  

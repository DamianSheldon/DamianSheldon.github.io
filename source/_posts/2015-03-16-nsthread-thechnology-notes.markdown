...
在非并发应用中只有一个运行线程。它在应用的主程序中开始和结束并一个一个地调用不同的方法或函数实现应用的行为。与些相反，支持并发的应用以一个线程开始，根据需要添加创建附加执行路径。每个新路径有它自定义的开始程序，它和应用的主程序是独立运行的。拥有多线程的应用提供两个重要的潜在优势：
...
POSIX 条件锁需要同时使用条件数据结构和互斥锁。虽然两种锁结构是分享的，互斥锁在运行时密切地捆绑到条件结构。

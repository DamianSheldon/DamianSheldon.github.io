...
Dispatch groups 是一种阻塞线程直到一个或多个任务完成运行的方法。当所有指定任务没有完成时你不能继续的地方可以使用这种行为。例如，在分发多个任务去计划某些数据，你也许会使用一个组来等待这些任务，当他们完成时处理它们的结果。另外一种使用 dispatch groups 的方法是把它当作 thread join 的替代品。与其启动多个子线程然后联接它们，你可以添加相关任务到 dispatch groups 然后等待整个组。
